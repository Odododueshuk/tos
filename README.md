...
## 求解模式與自選 Combo 與消珠顆數功能

專案內建強大的多重轉珠演算法，支援靈活的消珠策略與自選 Combo / 顆數功能，可在 GUI 面板「轉珠策略」下拉選單中直接切換（或透過 `config.json` 的 `solve_mode` 進行設定）：
...
- **指定消珠顆數 (`exactly_orbs`)**：使用者手動輸入目標消珠總顆數（支援 **3~30 顆**）。演算法會精準調整消除版面，確保最終消除的符石總數量剛好等於指定顆數，是用來破解「指定消除 X 顆符石」盾的終極利器。
- `short_8c`：歷史相容模式，找到 8 Combo 以上的最短路徑即停。
- `priority_color`：目前等同 `full_board`。

> [!TIP]
> 轉珠助手 GUI 具備流暢的控制連動：
> - 當您在「轉珠策略」選單中選擇 **「至少指定 Combo」** 或 **「剛好指定 Combo」** 時，「目標 Combo」選擇框會自動亮起並啟用；
> - 當您選擇 **「指定消珠顆數」** 時，「目標顆數」手動輸入框會自動亮起並啟用；
> - 若選擇其他策略，無關的輸入欄位則會智慧型自動變灰停用，避免操作混淆。
...
- `target_orbs`：目標消珠總顆數（支援整數 `3` ~ `30`，預設為 `30`）。
